...
def build_plot_positions(points: np.ndarray, width: int, height: int, time_scale: float) -> np.ndarray:
    if points.size == 0:
        return np.zeros((0, 3), dtype=np.float32)

    xs = points[:, 0].astype(np.float32)
    ys = points[:, 1].astype(np.float32)
    ts_us = points[:, 2].astype(np.float32)
...
def find_temporal_neighbor_points(points: np.ndarray, radius: int) -> np.ndarray:
    ...
    # Window size > 1 means there is at least one other temporal neighbor.
...
def show_vispy_cloud(pos: np.ndarray, colors: np.ndarray, point_size: float) -> None:
    ...
    cam.center = (0, 120, 0)   # move origin lower on screen
...
def main() -> None:
    ...
    ap = argparse.ArgumentParser(
        description="Plot saved x/y/t events from events_xyt.tsv with VisPy"
    )
    ap.add_argument(
        "--input",
        default=str(script_dir / "events_xyt.tsv"),
        help="Path to x/y/t TSV file (default: events_xyt.tsv next to this script)",
    )
    ...
    ap.add_argument(
        ...
        metavar="R",
        help=(
            "Color a point red if it has a temporal neighbor in [t-R, t+R]. "
            "Set R >= 0."
        ),
    )
    ap.add_argument(
        ...
        metavar="R",
        help=(
            "Color a point blue if it has a spatial neighbor within XY radius R "
            "(Euclidean distance). Set R >= 0."
        ),
    )
    ap.add_argument(
        "--hide-noise",
    ...
    if args.filter_time is not None or args.filter_spatial is not None:
        ...
        if args.filter_time is not None:
            print(
                f"[INFO] Applying temporal filter with radius {args.filter_time}..."
            )
            temporal_mask = find_temporal_neighbor_points(points, args.filter_time)
            matched_mask |= temporal_mask
            print(
                f"[INFO] Temporal matches: {int(temporal_mask.sum())} / {len(points)}"
            )

        if args.filter_spatial is not None:
            print(
                f"[INFO] Applying spatial filter with XY radius {args.filter_spatial}..."
            )
            spatial_mask = find_spatial_neighbor_points(
                points, args.filter_spatial, width=W, height=H
            )
    ...
    pos = build_plot_positions(render_points, W, H, time_scale=args.time_scale)
    colors = build_colors(
        len(pos), filter_mode, render_temporal_mask, render_spatial_mask
    )
